...
async function fetchBookingRecipients(client: any, bookingId: number): Promise<Recipient[]> {
  const ownerQuery = `
...
async function fetchBookingsInWindow(client: any, minutesFrom: number, minutesTo: number) {
  const query = `
    SELECT 
      b.id,
      b.start_time,
...
function buildEmailSubject(prefix: string, roomTypeName: string) {
  return `${prefix}: ${roomTypeName}`;
}

function buildEmailBody(name: string | null, roomType: string, roomInstance: string, startIso: string, notice: string) {
...
async function sendRemindersForWindow(client: any, windowLabel: '30min' | '1day') {
  const windowMinutes: [number, number] = windowLabel === '30min' ? [29, 30] : [23 * 60, 24 * 60];
